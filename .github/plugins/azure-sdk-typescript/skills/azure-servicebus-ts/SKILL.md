...
const client = new ServiceBusClient(fullyQualifiedNamespace, (process.env.NODE_ENV === "development" ? new DefaultAzureCredential() : new ManagedIdentityCredential()));
```
...
1. **Use Entra ID auth** - Avoid connection strings in production. Use a specific credential (eg. `ManagedIdentityCredential` or `WorkloadIdentityCredential`) in production. Use `DefaultAzureCredential` only for local development.
